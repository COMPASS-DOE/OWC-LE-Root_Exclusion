...
####What is the expected CCV chk std concentration for all analytes?####
NOx_CCV <- 0.5 
NH3_CCV <- 1.0 
PO4_CCV <- 0.15 
SO4_CCV <- 
...
##Should I just read in all the data files at once? 
...
stds_NO2 <- stds_NO2 %>%
  mutate(`Conc` = case_when(
    Sample_Name == "Standard 0" ~  0.0,
    Sample_Name == "Standard 1" ~  0.0,
    Sample_Name == "1ppb NO2-N" ~ 1,
    Sample_Name == "2ppb NO2-N" ~ 2,
    Sample_Name == "5ppb NO2-N" ~ 5,
    Sample_Name == "10ppb NO2-N" ~ 10,
    Sample_Name == "25ppb NO2-N" ~ 25,
    Sample_Name == "50ppb NO2-N" ~ 50,
    Sample_Name == "75ppb NO2-N" ~ 75,
    Sample_Name == "150ppb NO2-N" ~ 150,
...
####Is the CCV check std for all analytes?####
...
SO4_chks$SO4_diff <- ((SO4_chks$Conc - SO4_CCV)/((SO4_chks$Conc + SO4_CCV)/2)) * 100
SO4_chks$SO4_diff_flag <-  ifelse(SO4_chks$SO4_diff <= chk_conc_flag, 'YES', 'NO, rerun')

NO2_chks$NO2_diff <- ((NO2_chks$Conc - NO2_CCV)/((NO2_chks$Conc + NO2_CCV)/2)) * 100
...
####Was CCB used as the blank for all analytes?####
...
####Is is normal for blanks to be negative?####
...
####I don't see any duplicates in this dataset?####
# 
...
####I don't see any matrix effect checks####
# 
...
####Need to know detection limit and top standard for all analytes####
